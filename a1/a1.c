...
void list_it(const char *path, const char *name, const int size)
{
    DIR *dir = NULL;
...
void list_rec(const char *path, const char *name, const int size)
{
    DIR *dir = NULL;
    struct dirent *entry = NULL;
    char fullPath[512];
    ...
    if(argc >= 2)
    {
        ...
        if(strcmp(argv[1], "list") == 0){
            ...
            if(strcmp(argv[2], "recursive") == 0){// daca vrem listare recursiva doar si recursive este al 3 lea argument 
                    ...
                    else if(strstr(argv[3], "size_smaller=") != NULL){// daca e recursiv si avem "name starts with.."
                        if(strstr(argv[4], "path=") - argv[4] == 0 ){
                            printf("SUCCESS\n");
                            //list_rec(argv[4]+5, 0, argv[3]+17);
                        }
                    }
